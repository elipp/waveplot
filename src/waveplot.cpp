...
triangle* bakeWaveVertexArray(float* samples, const std::size_t& samplecount) {
	...
	float x1 = tmpx;
	float y1 = half_WIN_H*(samples[0]+1.0);

	float x2 = tmpx + step;
	float y2 = half_WIN_H*(samples[1]+1.0);

	float x3 = x2 + step;
	float y3 = half_WIN_H*(samples[2]+1.0);
	...
	for (int i = 1; i < samplecount;) 
	{
		...
		x1 = tmpx;
		y1 = half_WIN_H*(samples[i]+1.0);
		x2 = tmpx + step;
		y2 = half_WIN_H*(samples[i+1]+1.0);
		x3 = x2 + step;
		y3 = half_WIN_H*(samples[i+2]+1.0);
...
#endif
	uniform_texture1_loc = glGetUniformLocation(programHandle, "texture1");
